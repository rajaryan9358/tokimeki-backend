...
class userController extends BaseController
{
    ...
    public function __construct()
    {
        ...
        $this->app_token = new AppTokenModel();
    }
    ...
    {      
        ...
        foreach($res as $key){
            ...
            // print_r($res1);
            // die();
            $count_connection = $this->user->query("select COUNT(receiver_id)AS ct FROM `user_request` WHERE status = 'accept' AND sender_id=".$key->id." GROUP BY sender_id");
            $total_connection  = $count_connection->getResult();
           // echo "KEY=>".$key->id;
            $ct = 0;
            if ($total_connection) 
            {
                $ct = $total_connection[0]->ct;
            }


             $count_sent_messages = $this->user->query("select COUNT(id)AS cnt_message FROM `chat_messages` WHERE sender_id=".$key->id." GROUP BY sender_id");
            $total_sent_messages  = $count_sent_messages->getResult();
           // echo "KEY=>".$key->id;
            $cnt_sent_message = 0;
            if ($total_sent_messages) 
            {
                $cnt_sent_message = $total_sent_messages[0]->cnt_message;
            }

            $count_receive_messages = $this->user->query("select COUNT(id)AS cnt_message FROM `chat_messages` WHERE receiver_id=".$key->id." GROUP BY receiver_id");
            $total_receive_messages  = $count_receive_messages->getResult();
           // echo "KEY=>".$key->id;
            $cnt_receive_message = 0;
            if ($total_receive_messages) 
            {
                $cnt_receive_message = $total_receive_messages[0]->cnt_message;
            }

            $last_connection = $this->user->query("select ur.receiver_id,u.first_name, u.last_name FROM `user_request` as ur LEFT JOIN user AS u ON u.id = ur.receiver_id WHERE ur.status = 'accept' AND ur.sender_id=".$key->id." ORDER BY ur.id DESC LIMIT 1");
            $last_connection  = $last_connection->getResult();
            if (count($last_connection)>0) 
            {
               $last_connection_user = $last_connection[0]->first_name.'&nbsp'.$last_connection[0]->last_name;
            }
            else
            {
                $last_connection_user = "Not Available";
            }
            //print_r($last_connection);
        

            $action = "<td class='text-center'><ul class='icons-list'><li class='dropdown'><a href='#' class='dropdown-toggle' data-toggle='dropdown'><i class='icon-menu9'></i></a><ul class='dropdown-menu dropdown-menu-right'><li><a href='#'  onclick='change_status(".$key-> id . ',' . $key->is_active. ")'><i class='icon-pencil4'></i>". lang('Validation.change_status') ."</a></li><li><a href='#'  data-toggle='modal' data-target='#modal_default' onclick='view_user_details(".$key-> id.")'><i class='icon-list3'></i>". lang('Validation.view_details') ."</a></li><li><a onclick='delete_user(" . $key->id . ")'><i class='icon-trash'></i>" . lang('Validation.delete') . "</a></li></ul></li></ul></td>";
            ...
            // if ($res1['total_connection']=="") 
            // {
            //     $total_connection = 0;
            // }
            // else
            // {
            //     $total_connection = $res1['total_connection'];
            // }
            $data[] = array(
                ...
                'total_connection'=>$ct,
                'total_sent_messages'=>$cnt_sent_message,
                'total_receiver_messages'=>$cnt_receive_message,
                'last_connection'=>$last_connection_user,
                "action" => $action
            );
        }
//         echo "<pre>";
// print_r($data);
    ...
    public function fetchUserDetails()
    {
        ...
        if ($this->request->getVar('uid') != "") {
          ...
          $res = $this->user->select('user.*,user_points.point')->join('user_points','user_points.user_id = user.id')->where('user.id', $this->request->getVar('uid'))->first();
            ...
            $count_connection = $this->user->query("select COUNT(receiver_id)AS ct FROM `user_request` WHERE status = 'accept' AND sender_id=".$this->request->getVar('uid')." GROUP BY sender_id");
            $total_connection  = $count_connection->getResult();
           // echo "KEY=>".$key->id;
            $ct = 0;
            if ($total_connection) 
            {
             ...
             $count_sent_messages = $this->user->query("select COUNT(id)AS cnt_message FROM `chat_messages` WHERE sender_id=".$this->request->getVar('uid')." GROUP BY sender_id");
            $total_sent_messages  = $count_sent_messages->getResult();
           // echo "KEY=>".$key->id;
            $cnt_sent_message = 0;
            if ($total_sent_messages) 
            ...
            $count_receive_messages = $this->user->query("select COUNT(id)AS cnt_message FROM `chat_messages` WHERE receiver_id=".$this->request->getVar('uid')." GROUP BY receiver_id");
            $total_receive_messages  = $count_receive_messages->getResult();
           // echo "KEY=>".$key->id;
            $cnt_receive_message = 0;
            if ($total_receive_messages) 
            {
             ...
             $last_connection = $this->user->query("select ur.receiver_id,u.first_name, u.last_name FROM `user_request` as ur LEFT JOIN user AS u ON u.id = ur.receiver_id WHERE ur.status = 'accept' AND ur.sender_id=".$this->request->getVar('uid')." ORDER BY ur.id DESC LIMIT 1");
            $last_connection  = $last_connection->getResult();
            if (count($last_connection)>0) 
            {
               $last_connection_user = $last_connection[0]->first_name.'  '.$last_connection[0]->last_name;
            }
            else
            {
                $last_connection_user = "Not Available";
            }
            $res['last_connection']     = $last_connection_user;
            $res['total_connection']    = $ct;
            $res['cnt_sent_message']    = $cnt_sent_message;
            $res['cnt_receive_message'] = $cnt_receive_message;
            //print_r($res);
            if($res['profile_image'])
            {
               $img_path = '<div class="thumb thumb-rounded thumb-slide"><img src='. UPLOAD_IMG_PATH . $res['profile_image'] . ' alt="" style="height: 120px;"> <div class="caption-overflow"><span><a href='. UPLOAD_IMG_PATH . $res['profile_image'] . ' data-popup="lightbox" class="btn border-white text-white btn-flat btn-icon btn-rounded"><i class="icon-plus3"></i></a></span></div></div>';
            }
            else
            {
                $img_path = '<a href="#" class="media-left"><img src="'. base_url() .'/assets/images/user_placeholder.jpg" style="width: 120px;height: 120px;border-radius: 70%;"></a>';
            }
            $res['profile_image'] = $img_path;
     ...
     public function delete_user()
    {
        ...
        $res = $this->user->set(['is_delete' => 1])->where('id', $this->request->getVar("id"))->update();

        if ($res > 0) {
            $res = $this->app_token->set(['is_delete' => 1])->where('user_id', $this->request->getVar("id"))->update();
            $data[ERROR_CODE] = 1;
